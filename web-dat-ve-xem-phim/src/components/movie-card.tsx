...
export function MovieCard({ movie }: MovieCardProps) {
  return (
    <article className="overflow-hidden rounded-[28px] border border-white/10 bg-white/5 shadow-glow transition duration-300 hover:-translate-y-1 hover:border-white/20 hover:bg-white/8">
      <img src={movie.posterUrl} alt={movie.title} className="h-80 w-full object-cover" />
      <div className="space-y-4 p-5">
        ...
        <div>
          ...
          <p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-300">{movie.synopsis}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 text-xs text-slate-200">
          <span className="rounded-full bg-white/10 px-3 py-1">{movie.duration} phút</span>
          <span className="rounded-full bg-white/10 px-3 py-1">{movie.isNowShowing ? 'Đang chiếu' : 'Sắp chiếu'}</span>
        </div>
