...
export default async function HomePage() {
  ...
  const [nowShowing, upcoming, showtimes, heroMovie] = await Promise.all([
    prisma.movie.findMany({ where: { isNowShowing: true } }),
    prisma.movie.findMany({ where: { isComingSoon: true } }),
    prisma.showtime.count(),
    prisma.movie.findFirst({ where: { isNowShowing: true }, orderBy: { createdAt: 'asc' } })
  ]);
    ...
    <main className="page-shell py-12 lg:py-16">
      <section className="grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
        <div className="space-y-8">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              ...
              { label: 'Suất chiếu', value: showtimes.toString() }
            ].map((item) => (
        ...
        <div className="rounded-[32px] border border-white/10 bg-white/5 p-5 shadow-glow">
          {heroMovie ? (
            <img src={heroMovie.posterUrl} alt={heroMovie.title} className="h-[520px] w-full rounded-[28px] object-cover" />
          ) : (
