...
export default async function MovieDetailPage({ params }: MovieDetailPageProps) {
  ...
  const movie = movies.find((item) => item.slug === resolvedParams.slug);

  if (!movie) {
    notFound();
  }

  const relatedShowtimes = showtimes.filter((showtime) => showtime.movieSlug === movie.slug);
      ...
      <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-[24px] border border-white/10 bg-white/5 p-5">
            <div className="text-base font-semibold text-white">Suất chiếu mẫu</div>
            <div className="mt-4 grid gap-3">
              {relatedShowtimes.map((showtime) => (
                <div key={showtime.id} className="flex flex-col gap-3 rounded-2xl border border-white/10 bg-slate-950/60 p-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <div className="font-semibold text-white">{showtime.cinemaName}</div>
                    <div className="mt-1 text-sm text-slate-400">
                      {showtime.hallName} · {showtime.format} · {showtime.language}
                    </div>
                  </div>
